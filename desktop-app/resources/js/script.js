...
This is a fully client-side application. Your content never leaves your browser and stays secure on your device.`;
  ...
  // Global function to open a markdown file from external sources (e.g., main.js via file association)
  // Clears welcome tab and creates a new tab with the file content
  window.openFile = function(content, fileName) {
    // Check if we have a "Welcome to Markdown" tab, and replace it
    // Use loose matching: just check the name, content may have been edited
    var welcomeTabIndex = tabs.findIndex(function(t) { return t.name === 'Welcome to Markdown'; });
    if (welcomeTabIndex !== -1) {
      // Replace the welcome tab with the new file
      var tab = createTab(content, fileName);
      tabs[welcomeTabIndex] = tab;
      activeTabId = tab.id;
      saveTabsToStorage(tabs);
      saveActiveTabId(activeTabId);
      ...
      // No welcome tab found, create a new tab
